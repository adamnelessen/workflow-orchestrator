...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_state_manager_in_memory_mode():
    ...
    retrieved = await state.get_workflow("test-workflow-1")
    ...
    # Test job retrieval
    retrieved_job = await state.get_job("test-job-1")
...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_state_manager_job_assignments():
    """Test job assignment tracking"""
    state = StateManager()

    # Test job assignments
    await state.assign_job("job-1", "worker-1")
    await state.assign_job("job-2", "worker-1")
    await state.assign_job("job-3", "worker-2")

    # Test retrieval
    assert await state.get_job_worker("job-1") == "worker-1"
